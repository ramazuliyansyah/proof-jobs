...
export default async function JobDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      {/* Conditionally render the image section if an image path exists for the job */}
      {job.image && (
        <div className="mb-8 w-full max-w-2xl mx-auto"> {/* Wrapper for responsive image sizing */}
          <Image
            src={job.image} // Source path for the image (from public/images)
            alt={job.title} // Accessible alternative text for the image
            width={700}     // Intrinsic width (used for aspect ratio calculation and image optimization)
            height={450}    // Intrinsic height (used for aspect ratio calculation and image optimization)
            // 'layout' and 'objectFit' props are deprecated in Next.js 13+ 'Image' component.
            // Sizing and fitting are now handled via standard CSS (Tailwind classes here).
      ...
      <div className="mb-8">
        ...
        <ul className="list-disc list-inside text-gray-700 space-y-1"> {/* List of skills */}
          {job.skills.map((skill, index) => (
            <li key={index}>{skill}</li> // Each skill as a list item
          ))}
        </ul>
      </div>

      {/* Back Button to Jobs List */}
      <Link href="/jobs" passHref> {/* 'passHref' ensures href is passed to the underlying Button component */}
